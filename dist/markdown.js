...
exports.createMeetingMarkdown = createMeetingMarkdown;
...
function extractPanels(document) {
    const panels = {};
    for (const key of ['panels', 'ai_panels', 'generated_panels']) {
    ...
    if (lastViewedPanel && typeof lastViewedPanel === 'object') {
        ...
        const content = lastViewedPanel.content || lastViewedPanel.text || lastViewedPanel.markdown;
        if (content) {
...
function createMeetingMarkdown(document, transcript) {
    ...
    let attendees = document.attendees || [];
    if (attendees.length === 0) {
        attendees = document.google_calendar_event?.attendees || [];
    }
    if (attendees.length === 0) {
        attendees = document.people || [];
    }
    let attendeesStr;
    if (attendees.length > 0) {
    ...
    if (Object.keys(panels).length > 0) {
        for (const [panelName, content] of Object.entries(panels)) {
            if (panelName === 'notes' || panelName === 'summary')
                continue;
